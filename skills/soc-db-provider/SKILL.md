...
- `SESSION_ID`: Unique namespace for the current investigation session. **DO NOT MODIFY THIS VALUE.**
...
- **Examples:** `SESS-20260414-90058-TRIAGE-01`, `SESS-20260414-90058-ANALYSIS-05`.

### 2. Session Consistency (CRITICAL)
- **NO MODIFICATION:** You are STRICTLY FORBIDDEN from adding your agent name, status, or any other suffix to the **`session_id`** column. 
- You MUST use the **exact, literal value** found in the `SESSION_ID` environment variable for every write.
...
- **NO GUESSING:** You are STRICTLY FORBIDDEN from using hardcoded table names like `investigation_timeline`. You MUST use the EXACT values provided in the **`TIMELINE_DATA_TABLE`**, **`IOC_DATA_TABLE`**, and **`TUNING_DATA_TABLE`** environment variables.
...
- **To Log Activity:** Use `mcp_GoogleSecOps_add_rows_to_data_table` targeting the table name in **`TIMELINE_DATA_TABLE`**.
  **Mandatory JSON Template:**
  ```json
  {
    "session_id": "VALUE_OF_SESSION_ID",
...
### 6. Standardized Data Taxonomy (Strict Auditing)
...
### 8. Special Handling: Branching (Dolt Only)
